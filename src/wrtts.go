...
func main() {
	...
	if len(os.Args) == 3 {
		speaker = os.Args[1]
		text = os.Args[2]
	} else if (len(os.Args) == 4) && (os.Args[3] == "true") {
		speaker = os.Args[1]
		text = os.Args[2]
		intkey = true
	} else if (len(os.Args) == 4) && (os.Args[3] == "false") {
		speaker = os.Args[1]
		text = os.Args[2]
		intkey = false
	} else {
	...
	myAgi.Verbose(playback(speaker, text, audioformat, intkey, myAgi))
}

//play text to asterisk
func playback(speaker string, text string, format string, intkey bool, myAgi *agi.Session) string {

	//store a hashed verstion of our message to use as a filename
	name := hashString(speaker, text)
	...
	if alreadyCached(name, format) == false {
		mp3Name := getText2Speach(speaker, text, name)
	...
	if intkey == true {
		rep, err = myAgi.StreamFile(workDir+astName, "123456789")
...
func hashString(speaker string, text string) string {
	return fmt.Sprintf("MAKECALL"+speaker+"%x", md5.Sum([]byte(text)))
}
...
//do our text 2 speach dealie... retrieved as an mp3 TTS OLSSOO
func getText2Speach(speaker string, text string, filename string) string {
